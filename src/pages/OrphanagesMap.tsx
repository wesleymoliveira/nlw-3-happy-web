...
function OrphanagesMap() {
  ...
  function callPeer(id: any) {
    if(id!=='' && users[id] && id!==yourID){
      ...
      .catch(()=>{
        console.log('Por favor, verifique se possui (Câmera, Microfone). Certifique-se de dar as devidas permissões de acesso para o navegador.')
        
      })
    } else {
      console.log('Opa, usuário não encontrado, tem certeza que digitou corretamente?')
    ...
    incomingCall = (
      <div className="incomingCallContainer">
        <div className="incomingCall">
          ...
          <button name="accept" className="buttonAccept" onClick={()=>acceptCall()}>Aceitar</button>
          <button name="reject" className="buttonReject" onClick={()=>rejectCall()}>Recusar</button>
          </div>
        </div>
      </div>
    )
  ...
  let audioControl;
  if(audioMuted){
    audioControl=<span className="iconContainer" onClick={()=>handleClickMuteAudio()}>
  ...
  let videoControl;
  if(videoMuted){
    videoControl=<span className="iconContainer" onClick={()=>handleClickMuteVideo()}>
  ...
  <div id="page-map" style={{display: renderMap()}}>
   <aside>
     ...
     <footer>
       ...
       <div className="usernameActionText">Para quem deseja ligar, <span className="username highlight">{yourID}</span> ?</div>
       <input type="text" placeholder="ID do parceiro" value={receiverID} onChange={e => setReceiverID(e.target.value)}/>
       <button onClick={() => callPeer(receiverID.toLowerCase().trim())} >Ligar</button>
     </footer>
   </aside>

   <Map
     center={[-22.3016525, -42.5411443]}
     zoom={15}
